<?php 
	...
	$query = mysqli_query($koneksi, "INSERT INTO t_air_bersih VALUES ('$kode','$tgl','$kodek','$namap','$koded','$perihal','$no','$lokasi','$syarat','$daerah','$pengirim','$diambil','$diterima','$diperiksa','$kekeruhan','$jumlahzat','$suhu','$derajat','$besi','$fluorida','$caco3','$mangan','$no3','$no2','$sulfat','$kromium','$klorida','$tembaga','$alumunium')")or die(mysqli_error($koneksi));

	if ($query) {
		echo "<script>window.alert('Data Berhasil Di input')
				      window.location='air_bersih.php'</script>";
	}
