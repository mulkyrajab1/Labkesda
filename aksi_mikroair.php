...
$query = mysqli_query($koneksi, "INSERT INTO m_air VALUES ('$id','$tgl','$kodek','$namap','$pengirim','$diambil','$diterima','$diperiksa','$hal','$no','$lokasi','$titik','$syarat')")or die(mysqli_error($koneksi));

$jumlah_dipilih = count($parameter);

 for ($i=0;$i< $jumlah_dipilih;$i++) { 
 $query2 = mysqli_query($koneksi, "INSERT INTO detail_mikro_air VALUES ('$id2','$id','$parameter[$i]','$kadar[$i]','$satuan[$i]','$hasil[$i]','$metode[$i]')")or die(mysqli_error($koneksi));
